...
/**
 * [processDataToDrawInPieChartWithFilters description]
 * @param  {[type]} filter [description]
 * @return {[type]}        [description]
 */
processDataToDrawInPieChartWithFilters = function (filter) {
  var arrayElements = [];
  arrayDataApi.forEach(function (element) {
    //console.log(element.user_agent.match(/Safari|Chrome|Firefox/g));
    if(filter==='total'){
      arrayElements.push(element.type);
    }
    if(filter==='online' && element.type==='chat'){
      arrayElements.push(element.type);
    }
    if(filter==='offline' && element.type==='offline'){
      arrayElements.push(element.type);
    }
  });
  var uniqueValues = _.uniq(arrayElements);
  var arrayToDraw = [];
  uniqueValues.forEach(function (element) {
    //11.01.2015 LFG count the number of times that one element appears in the array
    arrayToDraw.push([element, arrayElements.filter(function (x) {return x==element;}).length]);
  });
  return arrayToDraw;
};
...
processDataToDrawTheColumnsCharts = function (kindOfChart) {
  ...
  arrayDataApi.forEach(function (chat) {
    //11.01.2015 LFG 1000 is not a magic number, is for converting the dates in seconds to milliseconds
    datesChart.push(moment(chat.created_at*1000).format("MMM Do YY"));
